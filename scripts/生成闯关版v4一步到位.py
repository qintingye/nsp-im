#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""v4 final 闯关版 · 70题全有答案"""
import openpyxl, json
from pathlib import Path
from collections import Counter

XLSX = Path(r'C:\Users\Administrator\AppData\Local\hermes\cache\documents\doc_354b3f59dd91_新型电力系统岗位题库-更新(1).xlsx')
...
for s in wb.sheetnames:
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        ...
        opts = []
        for j in range(8):
            v = sg(row, IDX['opts_start'] + j)
            if v: opts.append(v)
        ans = sg(row, IDX['ans'])
        qs.append({
            'id': rid,
            'cat': sg(row, IDX['cat']) or '',
            'type': sg(row, IDX['type']) or '单选题',
            'diff': sg(row, IDX['diff']) or '简单',
            'q': (sg(row, IDX['q']) or '').replace('\r\n',' ').replace('\n',' ').strip(),
            'opts': opts,
            'answer': ans if isinstance(ans, str) else (''.join(ans) if ans else ''),
            'explain': (sg(row, IDX['exp']) or '暂无解析').strip(),
        })

type_cnt = Counter(q['type'] for q in qs)
print(f'题数: {len(qs)}, 有答案: {sum(1 for q in qs if q["answer"])}')
print(f'题型: {dict(type_cnt)}')

ALL_JSON = json.dumps(qs, ensure_ascii=False)
...
.header .badge { background: linear-gradient(135deg, #10b981, #059669); color: white; padding: 4px 10px; border-radius: 12px; font-size: 11px; font-weight: 600; }
...
.level-card { background: linear-gradient(135deg, #ecfdf5, #a7f3d0); margin: 12px 16px; padding: 16px; border-radius: 12px; border-left: 4px solid #10b981; }
.level-title { font-size: 14px; color: #065f46; font-weight: 600; margin-bottom: 4px; }
.level-subtitle { font-size: 12px; color: #047857; }
...
.option:hover:not(.locked) { border-color: #10b981; background: #f0fdf4; transform: translateX(2px); }
.option.selected { border-color: #10b981; background: #f0fdf4; }
...
.option.selected .opt-letter { background: #10b981; color: white; border-color: #10b981; }
...
.btn-next { background: #2563eb; color: white; animation: pulse 2s infinite; }
...
.win-modal { position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: linear-gradient(135deg, rgba(16,185,129,0.95), rgba(5,150,105,0.95)); display: none; align-items: center; justify-content: center; z-index: 200; }
...
.win-title { font-size: 26px; font-weight: 800; background: linear-gradient(135deg, #10b981, #059669); -webkit-background-clip: text; -webkit-text-fill-color: transparent; margin-bottom: 8px; }
...
.win-box button { background: linear-gradient(135deg, #10b981, #059669); color: white; border: none; padding: 14px 28px; border-radius: 8px; font-size: 15px; font-weight: 700; cursor: pointer; }
...
.update-badge { background: #d1fae5; border: 1px solid #6ee7b7; color: #065f46; padding: 8px 12px; margin: 12px 16px; border-radius: 6px; font-size: 12px; line-height: 1.5; }
</style>
</head>
<body>
...
题库已更新 · 2026-08-25<br>
共 <b>__TOTAL__ 题</b>（<b>全部有答案</b>）· v4 完整版
</div>
...
function renderQuestion() {
  ...
  var isMulti = q.type === '多选题' || q.type === '不定项选择题';
  locked = false;
  selectedIdxs = [];
  ...
  var html = '<div class="quiz-section"><div class="q-header">';
  html += '<span class="q-type ' + (isMulti ? 'multi' : '') + '">' + q.type + (isMulti ? '（多选）' : '') + '</span>';
  html += '<span class="q-meta">📊 ' + q.difficulty + '</span></div>';
  html += '<div class="q-text">' + q.q + '</div><div class="options" id="options">';
  for (var idx = 0; idx < q.opts.length; idx++) {
    var letter = String.fromCharCode(65 + idx);
    html += '<div class="option" onclick="selectOption(' + idx + ',' + (isMulti ? 'true' : 'false') + ')" data-idx="' + idx + '">';
    html += '<div class="opt-letter">' + letter + '</div>';
    html += '<div class="opt-text">' + q.opts[idx] + '</div></div>';
  }
  html += '</div><div class="feedback" id="feedback"></div><div class="actions">';
  if (isMulti) html += '<button class="btn-submit" id="btn-submit" onclick="submitMulti()" disabled>提交答案</button>';
  html += '<button class="btn-next" id="btn-next" onclick="nextQuestion()" disabled>';
  html += (current === QUESTIONS.length - 1 ? '🏁 完成' : '下一关 →') + '</button></div></div>';
...
function judgeSingle() {
  ...
  var isCorrect = selectedIdxs.length === correctIdxs.length && selectedIdxs.every(function(i) { return correctIdxs.indexOf(i) >= 0; });
...
function handleCorrect(q, correctIdxs, opts) {
  ...
  opts.forEach(function(o, idx) {
    ...
    if (correctIdxs.indexOf(idx) >= 0) o.classList.add('correct-show');
  });
  passed++;
  streak++;
  ...
  var explainHtml = q.explain && q.explain !== '暂无解析' ? '<div class="reason">💡 ' + q.explain + '</div>' : '';
  fb.innerHTML = '<b><span class="icon">✅</span>闯关成功！答案：' + q.answer + '</b>' + explainHtml;
...
function handleWrong(q, correctIdxs, opts) {
  ...
  opts.forEach(function(o, idx) {
    o.classList.add('locked');
    if (selectedIdxs.indexOf(idx) >= 0 && correctIdxs.indexOf(idx) < 0) o.classList.add('wrong');
    if (correctIdxs.indexOf(idx) >= 0) o.classList.add('correct-show');
  });
  var correctSet = new Set(correctIdxs);
  var selectedSet = new Set(selectedIdxs);
  ...
  var explainHtml = q.explain && q.explain !== '暂无解析' ? '<br><br>💡 解析：' + q.explain : '';
  document.getElementById('fail-text').innerHTML = reason + '<br><br>正确答案：<b>' + q.answer + '</b>' + explainHtml + '<br><br>📍 闯关失败将<b>回到第 ' + targetLevel + ' 关</b>重新挑战';
...
html = HTML.replace('__TOTAL__', str(len(qs)))
